...
def validate_sensor_data_config(sensor_config: Dict[str, Any], mode: str) -> None:
    ...
    if "batch_loader" in sensor_config:
        ...
        # data_root と dataset_location_pairs のチェック（pretrainで使用）
        ...
        # data_root + datasets + sensor_location のチェック（finetuneで使用）
        elif "data_root" in sensor_config and "datasets" in sensor_config and "sensor_location" in sensor_config:
        ...
        # dataset_patterns のチェック（従来の設定方式 - 後方互換性のため残す）
            ...
            raise ConfigValidationError(
                "Either 'data_root' + 'dataset_location_pairs' (pretrain) or "
                "'data_root' + 'datasets' + 'sensor_location' (finetune) or "
                "'dataset_patterns' must be provided in config."
            )
