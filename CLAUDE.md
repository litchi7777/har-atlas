...
**重要**: トレーニングやテストなどのコード実行は、ユーザーが直接行います。

- ❌ Claudeは`python train.py`などの実行コマンドを実行しない
- ✅ Claudeは設定ファイルの準備、コードの修正、説明のみを行う
- ✅ 実行準備が整ったら、ユーザーに実行方法を提示する

**例外**: ファイルの存在確認、git操作、簡単な検証スクリプトなど、副作用の少ないコマンドは実行可能
